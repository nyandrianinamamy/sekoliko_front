...
export class EditUserComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        /**
         * Gestion role , raha misy mivaha ity dia andefaso mp julien rajerison
         */
        if (role.role_type.id !== ConstantRole.ADMIN && role.role_type.id != ConstantRole.SECRETARIAT &&
            role.role_type.id != ConstantRole.PROFS && role.role_type.id != ConstantRole.ETUDIANT) {
            this.router.navigate(['/menu/not-found']);
        }
        if (role.role_type.id === ConstantRole.PROFS && role.role_type.id != ConstantRole.ETUDIANT) {
            this.href = this.currentRoute.snapshot.paramMap.get('id');
            let usId = role.user_id;
            if (this.href != usId) {
                this.router.navigate(['/menu/not-found']);
            }
        }
        ...
        if (this.updateAction) {
            this.dataService.post(urlList.path_user_find + '/' + this.userId, etudiant).subscribe(response => {
                if (response.code === ConstantHTTP.CODE_SUCCESS) {
                    ...
                    } else if (role.role_type.id === ConstantRole.ETUDIANT) {
                        this.router.navigate(['/menu/profile'])
                    } else {
